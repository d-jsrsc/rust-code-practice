...
pub fn binary_search_rec<T: Ord>(
    ...
    match target.cmp(&list_of_items[mid]) {
        Ordering::Equal => Some(mid),
        Ordering::Greater => binary_search_rec(list_of_items, target, &(mid + 1), right),
        Ordering::Less => binary_search_rec(list_of_items, target, left, &mid),
    }
}
